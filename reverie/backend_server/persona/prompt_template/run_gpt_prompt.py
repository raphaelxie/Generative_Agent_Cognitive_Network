...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 5, 
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 500, 
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 30, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, 
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 40, 
               "temperature": 0.7, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  # gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 3, 
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 150, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona,
  ...
  # gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 2000, 
  #              "temperature": 0.7, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_csn_connection(persona, statements, person_j, person_k, test_input=None, verbose=False):
  """
  Cognitive social network (CSN) survey: does person_j have a connection with person_k?
  Returns 0, 1, or None (unsure). Used for Krackhardt-style cognitive social structure.
  """
  def create_prompt_input(persona, statements, person_j, person_k, test_input=None):
    if test_input:
      return test_input
    return [statements, persona.scratch.name, person_j, person_k]

  def __csn_clean_up(gpt_response, prompt=""):
    s = gpt_response.strip().lower()
    if s in ("0", "no"):
      return "0"
    if s in ("1", "yes"):
      return "1"
    if s in ("null", "unsure", "unknown", ""):
      return "null"
    if s.startswith("0") or s.startswith("1"):
      return s[0]
    return "null"

  def __csn_validate(gpt_response, prompt=""):
    try:
      __csn_clean_up(gpt_response, prompt)
      return True
    except Exception:
      return False
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 5,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/csn_connection_v1.txt"
  prompt_input = create_prompt_input(persona, statements, person_j, person_k, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "1"
  special_instruction = "The output must be exactly one of: 0, 1, or null."
  fail_safe = get_fail_safe()
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __csn_validate, __csn_clean_up, True)
  if output is not False:
    out_val = None if output == "null" else int(output)
    return out_val, [output, prompt, gpt_param, prompt_input, fail_safe]
  return None, [None, prompt, gpt_param, prompt_input, fail_safe]


def run_gpt_prompt_csn_connection_batch(persona, statements, person_j, list_person_k, test_input=None, verbose=False):
  """
  CSN survey batch: for one person_j, answer connection with each person in list_person_k.
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 120,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/csn_connection_batch_v1.txt"
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 250, 
               "temperature": 1, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
##############################################################################
# Network Cognition (NCN) Perception Survey Prompts
##############################################################################

def run_gpt_prompt_ncn_centrality_rank(persona, statements, roster_names,
                                       test_input=None, verbose=False):
  """
  NCN survey: rank the full roster (including respondent) from most to least
  socially connected, based on the respondent's memories.
  Returns a list of names in rank order (most connected first).
  """
...
def run_gpt_prompt_ncn_self_position(persona, statements, other_names,
                                     test_input=None, verbose=False):
  """
  NCN survey: rate own social connectedness on a 1-5 Likert scale,
  relative to the other people listed.
  Returns an int in [1, 5].
  """
  def create_prompt_input(persona, statements, other_names, test_input=None):
    if test_input:
      return test_input
    names_str = ", ".join(other_names)
    return [statements, persona.scratch.name, names_str]

  def __func_clean_up(gpt_response, prompt=""):
    digits = re.findall(r"[1-5]", gpt_response.strip())
    if digits:
      return int(digits[0])
    return 3
  ...
  def __func_validate(gpt_response, prompt=""):
    try:
      val = __func_clean_up(gpt_response, prompt)
  ...
  gpt_param = {"engine": DEFAULT_CHAT_MODEL, "max_tokens": 5,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/ncn_self_position_v1.txt"
  prompt_input = create_prompt_input(persona, statements, other_names, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "3"
